...
namespace P9_DecoratorLibrary
{
    ...
    public class LogDecorator : SerialCommDecorator
    {
        ...
        {
            // 记录发送日志（可写入本地文件或数据库）
            Console.WriteLine($"[日志装饰器] 记录发送日志：时间={DateTime.Now:HH:mm:ss}，数据={BitConverter.ToString(data)}");
            // 调用底层发送逻辑
        ...
        public override byte[] ReceiveData()
        {
            ...
            if (data != null)
            {
                Console.WriteLine($"[日志装饰器] 记录接收日志：时间={DateTime.Now:HH:mm:ss}，数据={BitConverter.ToString(data)}");
            }
            else
            {
                Console.WriteLine($"[日志装饰器] 记录接收日志：时间={DateTime.Now:HH:mm:ss}，数据=NULL（CRC校验失败）");
            }
            return data;
        }
